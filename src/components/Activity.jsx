...
function Activity() {
  ...
  const fetchData = async (currentPage) => {
    try {
      const response = await axios.get(
        `http://project-back-end.vercel.app/api/activities/?page=${currentPage}`
      );
  ...
  const update = (id, updatedList) => {
    const updatedActList = list.map((item) => {
      if (item.id === id) {
        ...
        axios
          .patch(`http://project-back-end.vercel.app/api/activities/${id}/`, editForm, {
            headers,
          })
          .then((res) => {
  ...
  const handleDeleteTrue = () => {
    if (popupDelete.show && popupDelete.id) {
      ...
      axios
        .delete(`http://project-back-end.vercel.app/api/activities/${popupDelete.id}/`)
        .then((res) => {
          setList(del);
          fetchData();
        });
