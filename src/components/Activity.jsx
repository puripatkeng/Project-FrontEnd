...
function Activity() {
  ...
  const fetchData = async (currentPage) => {
    try {
      ...
      const response = await axios.get(
        `https://project-back-end-puripatkeng.vercel.app/api/activities/?page=${currentPage}`
      );
  ...
  const handleDeleteTrue = () => {
    if (popupDelete.show && popupDelete.id) {
      ...
      axios
        .delete(`https://project-back-end-puripatkeng.vercel.app/api/activities/${popupDelete.id}/`)
        .then((res) => {
          setList(del);
          fetchData();
        });
